...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    """
    t1 = fs / float(f1)          # Period of the first sine in samples
    t2 = fs / float(f2)          # Period of the second sine in samples
    M = int((t1 * t2) / gcd(t1, t2))  # Analysis window in samples
    # If we assume M is even, those two have the same value, so we could use just one.
    hM1 = int(math.floor((M + 1) / 2))
    hM2 = int(math.floor(M / 2))
 
    # Truncate the signal to M samples
    x = x[:M]
    ...
    X = fft(fftbuffer)[:hM1+1]
    return 20 * np.log10(abs(X))


def generateSine(length, fs, f):
    """
    Generates length samples of a sine wave of frequency f, sampled at fs
    Inputs:
        lenght (int) = lenght of the sine to generate
        fs (int) = sampling frequency in Hz
        f (int) = frequency of the sine
    """
    return np.sin(2 * np.pi * f * np.arange(length) / fs)
...
if __name__ == '__main__':
    ...
    out = minimizeEnergySpreadDFT(x, fs, f1, f2)
