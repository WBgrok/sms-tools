...
def suppressFreqDFTmodel(x, fs, N):
    ...
    filtered_mX = mX.copy()
    index_70 = int(np.ceil(float(N) / float(fs) * 70))
    print index_70

    # dftAnal returns its spectrum in dB to zero a frequency, we set it to -120
    filtered_mX[:index_70 + 1] = -120

    y = outputScaleFactor * dftSynth(mX, pX, w.size)
    yfilt = outputScaleFactor * dftSynth(filtered_mX, pX, w.size)
    return (y, yfilt)
