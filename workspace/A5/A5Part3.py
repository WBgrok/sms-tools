...
def mainlobeTracker(inputFile = '../../sounds/sines-440-602-hRange.wav', window='boxcar', t=-80):
    """
    Input:
...
if __name__ == '__main__':
    for window in ['boxcar', 'hanning', 'hamming', 'blackman',  'blackmanharris']:
        ...
        t = 0
        meanErr = np.array([3, 3])
        while any(meanErr >= 2):
            
            meanErr = mainlobeTracker(window=window, t=t)
            t -= 10
            if t < -120:
                print "Breaking out!"
                brk = True
