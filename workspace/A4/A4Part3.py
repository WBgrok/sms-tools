...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    Hz_per_bin = fs / float(N)
    n_frames = mX.shape[0]

    i_low = int(round(low_cutoff / Hz_per_bin)) 
    i_hi = int(round(hi_cutoff / Hz_per_bin))
    print i_low
    print i_hi
    print n_frames

    EnLow = np.zeros(n_frames)
    EnHi = np.zeros(n_frames)

    for f in np.arange(n_frames):
        Xlow = mX[f, 1:i_low]
        Xhi = mX[f, i_low:i_hi + 1]
        EnLow[f] = np.sum(abs(10 ** (Xlow / 20)) ** 2)
        EnHi[f] = np.sum(abs(10 ** (Xhi / 20)) ** 2)
        if f == 0:
            print EnLow[f]
            print EnHi[f]

    EngEnv = np.zeros((n_frames, 2))
    EngEnv[:, 0] = 10 * np.log10(EnLow)
    EngEnv[:, 1] = 10 * np.log10(EnHi)
