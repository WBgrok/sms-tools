...
def computeSNR(inputFile, window, M, N, H):
    ...
    out = stft.stft(x, w, N, H)
    noise = x - out

    # wavwrite(out, fs, "%s_synth.%s" % (".".join(inputFile.split(".")[:-1]), inputFile.split(".")[-1]))
    # wavwrite(noise, fs, "%s_noise.%s" % (".".join(inputFile.split(".")[:-1]), inputFile.split(".")[-1]))
    ...
    SNR1 = 10 * np.log10(Ex / En)
    SNR2 = 10 * np.log10(Ex2 / En2)
